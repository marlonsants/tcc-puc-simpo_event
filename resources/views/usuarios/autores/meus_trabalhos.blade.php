		...
		<div class="col-xs-12 col-md-12">
			...
			<table class="table table-bordered table-responsive table-condensed table-bordered table-striped">
				...
				<tbody>
					...
					<tr>
						<td width="400">{{$trabalho->titulo}}</td>
						...
						<!-- @if($arrayDatas['dataAtual'] > $arrayDatas['data_fim_ava'] )
							<td style="font-size: 22px"><b>{{ number_format($trabalho->notaFinal, 2)}}</b></td>
						@else
						...
						@endif -->
							...
							@if($arrayDatas['dataAtual'] > $arrayDatas['data_fim_ava'] )
								...
								@if($trabalho->status_id == 1)
									...
									<form action="{{url('autor/trabalhos/visualizarComprovante')}}" method="post" style="display: inline-block;" target="_blank">
										{!!csrf_field()!!}
